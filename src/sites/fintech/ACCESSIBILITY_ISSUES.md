...
| ------ | --------- | -------------------------------------------------- | ------- | ----------------------------- | -------- | -------- | -------- |
| MT-001 | Home      | `Home.jsx` testimonial photo `<img>` in the `.testimonial-card` (Unsplash URL, no `alt` attribute) | 1.1.1   | `image-alt`                   | Critical | axe-core | Live     |
...
| MT-009 | Home      | `Home.jsx` promo-grid — four "Learn more" links on the credit-card-style promo cards | 2.4.4 | `identical-links-same-purpose` | Minor   | axe-core (**Needs Review** — incomplete result, requires human confirmation) | Live |
...
| MT-011 | Home      | `Home.jsx` "Cash offer up to $500" `<div>` styled like an h2 inside the `.cash-offer-card` | 1.3.1 | `advanced/heading-markup`      | Serious  | **Pro Advanced** (uses AI credits — CV) | Live |
...
| MT-016 | Statements| `Statements.jsx` statements `<table>` — `<th>` elements use invalid `scope="column"` | n/a (BP) | `scope-attr-valid`            | Serious  | axe-core (Best Practice) | Live     |
...
| MT-025 | Home      | `Home.jsx` "Filter by:" `<select>` above the promo-grid (no label / aria-label / aria-labelledby / title) | 4.1.2 | `select-name`                 | Critical | axe-core | Live |
| MT-026 | Home      | `Home.jsx` inline ascending bar-chart `<svg role="img">` next to the first promo-card badge (no `<title>`, no aria-label) | 1.1.1 | `svg-img-alt`                 | Serious  | axe-core | Live |
| MT-027 | Home      | `Home.jsx` "Popular products" `<div role="listbox">` below the login card whose children are plain `<div>`s (no `role="option"`) | 1.3.1 | `aria-required-children`      | Critical | axe-core | Live |
| MT-028 | Home      | `Home.jsx` promotional-rate stripe between the hero and the bottom grid — white text on a near-white / pale gold `linear-gradient` (~2.5–3.5:1) | 1.4.3 | `advanced/text-contrast` (axe-core `color-contrast` returns **Needs Review** because the bg is a gradient) | Serious | **Pro Advanced** | Live |
| MT-029 | Home      | `Home.jsx` `/branch-photo.svg` informative branch-building image in the `.branch-footer` served with `alt=""` | 1.1.1 | `advanced/image-informative-has-alt` | Minor | **Pro Advanced** (uses AI credits — image classifier) | Live |
| MT-030 | Home      | `Home.jsx` "Show additional login options" `<button aria-expanded="yes">` at the bottom of the login card — invalid value (valid: `true`/`false`) | 4.1.2 | `aria-valid-attr-value`       | Critical | axe-core | Live |
...
| MT-079 | Home      | `Home.jsx` `.back-to-top` link (SVG chevron only, `aria-hidden` on the svg, no accessible name) at the bottom of the branch footer | 2.4.4 | `link-name`                    | Critical | axe-core | Live |
| MT-080 | Home      | `Home.jsx` nested `<button>` (`.testimonial-preview-btn`) inside the "Read their stories" `<Link role="button">` in the testimonial card | 4.1.2 | `nested-interactive`           | Serious  | axe-core | Live |
| MT-081 | Home      | `Home.jsx` `.fine-print` "Terms and conditions apply…" line below the Open-an-account button — light grey on white (~2.4:1) | 1.4.3 | `color-contrast`               | Serious  | axe-core | Live |
| MT-082 | Home      | `Home.jsx` sticky-offer close button (icon-only SVG, no aria-label, aria-hidden svg) | 4.1.2 | `button-name`                  | Critical | axe-core | Live |
| MT-083 | Home      | `theme.css` `.btn-login:focus,.btn-login:focus-visible { outline: none }` removes the focus indicator on the Log in button with no replacement style | 2.4.7 | `advanced/css-focus-visible`   | Serious  | **Pro Advanced** | Live |
| MT-084 | Home      | `Home.jsx` `.featured-this-month` "Featured this month" styled `<div>` above the promo grid — reads as an h2 visually but not in the outline | 1.3.1 | `advanced/heading-markup`      | Serious  | **Pro Advanced** (uses AI credits — CV) | Live |
| MT-085 | Home      | `Home.jsx` `.archive-hint` container has `aria-hidden="true"` yet contains a focusable `<Link>` ("View last quarter's offers") | 4.1.2 | `aria-hidden-focus`            | Serious  | axe-core | Live |
| MT-086 | Home      | `Home.jsx` `.fine-print-more` "Read full terms" `<span tabIndex={0} onClick>` — focusable but no interactive role | 2.4.3 | `focus-order-semantics`        | Minor    | axe-core (BP) | Live |
| MT-087 | Checking  | `Checking.jsx` "Not sure which fee applies?" styled `<div>` above the fee table — reads as a heading visually but not in the outline | 1.3.1 | `advanced/heading-markup`      | Serious  | **Pro Advanced** (uses AI credits — CV) | Live |
| MT-088 | Checking  | `Checking.jsx` "See full fee glossary" `<span tabIndex={0} onClick>` — focusable but no interactive role | 2.4.3 | `focus-order-semantics`        | Minor    | axe-core (BP) — also caught by the automated Keyboard IGT's role-mismatch lens | Live |
| MT-089 | Savings   | `Savings.jsx` compounding-example banner — white text on a gold-to-white `linear-gradient` | 1.4.3 | `advanced/text-contrast` (axe-core `color-contrast` returns **Needs Review** because the bg is a gradient) | Serious | **Pro Advanced** | Live |
| MT-090 | Savings   | `Savings.jsx` "See a compounding example" `<span tabIndex={0} onClick>` styled as a solid pill BUTTON (no underline) — focusable but no interactive role; Keyboard IGT AI suggests `role="button"` | 2.4.3 | `focus-order-semantics`        | Minor    | axe-core (BP) — also caught by the automated Keyboard IGT's role-mismatch lens | Live |
| MT-091 | Credit Cards | `CreditCards.jsx` per-card `.card-apply-btn` (theme.css) strips the keyboard focus ring with no replacement visual state | 2.4.7 | `advanced/css-focus-visible`   | Serious  | **Pro Advanced** (no AI credits — CSS state diff) | Live |
| MT-092 | Credit Cards | `CreditCards.jsx` "Remember my card choice" `<div tabIndex={0} onClick>` styled as a square-box CHECKBOX (no `role="checkbox"`, no `aria-checked`) — Keyboard IGT AI suggests `role="checkbox"` | 2.4.3 | `focus-order-semantics`        | Minor    | axe-core (BP) — also caught by the automated Keyboard IGT's role-mismatch lens | Live |
| MT-093 | Mortgages | `Mortgages.jsx` `/branch-photo.svg` informative branch-building image served with `alt=""` | 1.1.1 | `advanced/image-informative-has-alt` | Minor | **Pro Advanced** (uses AI credits — image classifier) | Live |
| MT-094 | Mortgages | `Mortgages.jsx` "Rate lock terms" / "Extended lock options" segmented-control TAB pair (2 `<span tabIndex={0} onClick>`, no `role="tab"`/`tablist`, no `aria-selected`) — 2 tab stops, Keyboard IGT AI suggests `role="tab"` | 2.4.3 | `focus-order-semantics`        | Minor    | axe-core (BP) — also caught by the automated Keyboard IGT's role-mismatch lens (×2 nodes) | Live |
| MT-095 | Credit Cards | `CreditCards.jsx` `RatesTable id="compare"` collides with the card-grid section's existing `id="compare"` — both derive an `<h2 id="compare-heading">`, and both sections' `aria-labelledby` reference the now-ambiguous duplicate id | 4.1.1 | `duplicate-id-aria` (ships `reviewOnFail: true` in axe-core — always **Needs Review**, never an automatic violation, same bucket as MT-009) | Critical | axe-core (**Needs Review** — confirmed via Playwright DOM inspection; does not appear in this MCP's `analyze` output) | Live |
| MT-096 | Checking  | `Checking.jsx` icon-only info toggle — `<span role="button" tabIndex={0}>` wrapping only an `aria-hidden` SVG, no visible text/aria-label/title | 4.1.2 | `aria-command-name`            | Serious  | axe-core | Live |
| MT-097 | Savings   | `Savings.jsx` FDIC-insured shield `<svg role="img">` with no `<title>`/aria-label | 1.1.1 | `svg-img-alt`                  | Serious  | axe-core | Live |
| MT-098 | Mortgages | `Mortgages.jsx` icon-only help toggle next to "Extended lock options" — `<span role="button" tabIndex={0}>` wrapping only an `aria-hidden` SVG, no visible text/aria-label/title | 4.1.2 | `aria-command-name`            | Serious  | axe-core | Live |
| IGT-001 | Cards     | `Cards.jsx` top-level `<div role="presentation">` wrapper around page content | 1.3.1 | Structure IGT (`presentation-role-conflict` adjacent) | Moderate | **IGT** (Structure) | Live |
| IGT-002 | Statements | `Statements.jsx` SECOND `<main aria-label="Tax help tips">` wrapping a Tax tips callout | 1.3.1 | `landmark-one-main` (BP) + Structure IGT | Moderate | **IGT** (Structure) | Live |
| IGT-003 | Help      | `Help.jsx` orphan `<section>` (no aria-label, no aria-labelledby, no heading inside) | 1.3.1 | `region` (BP) + Structure IGT | Moderate | **IGT** (Structure) | Live |
| IGT-004 | Wealth    | `Wealth.jsx` new `<h4>` "A note on advisor availability" after hero `<h1>` (no h2/h3 between) | 1.3.1 | `heading-order` (BP) + Headings IGT | Moderate | **IGT** (Headings) | Live |
| IGT-005 | About     | `About.jsx` SECOND `<h1>` "DQBC, in a nutshell" after MarketingHero `<h1>` | 1.3.1 | `page-has-heading-one` (BP) + Headings IGT | Moderate | **IGT** (Headings) | Live |
| IGT-006 | Profile   | `Profile.jsx` empty `<h2></h2>` with no text content | 1.3.1 | `empty-heading` (BP) + Headings IGT | Minor   | **IGT** (Headings) | Live |
| IGT-007 | Login     | `Login.jsx` required "Security question" `<input required>` with no asterisk / aria-required / visible marker | 3.3.2 | Forms IGT (required-state visibility) | Moderate | **IGT** (Forms) | Live |
| IGT-008 | Profile   | `Profile.jsx` inline `<p>Email must include @</p>` error next to email field — NOT linked via aria-describedby | 3.3.1 | Forms IGT (error association) | Serious | **IGT** (Forms) | Live |
| IGT-009 | Transfer  | `Transfer.jsx` Step 1 three "Confirmation preferences" checkboxes with no `<fieldset>` / `<legend>` | 1.3.1 | Forms IGT (group labelling) | Moderate | **IGT** (Forms) | Live |
| IGT-010 | Dashboard | `Dashboard.jsx` two `<img>` with same src `/fintech-hero.svg` and same `alt="DQBC seal"` | 1.1.1 | `image-redundant-alt` (BP) + Images IGT | Minor   | **IGT** (Images) | Live |
| IGT-011 | About     | `About.jsx` `<img src="/branch-photo.svg" alt="Our story" />` — image-of-text whose alt doesn't match the visual text | 1.4.5 | Images IGT (image-of-text) | Serious | **IGT** (Images) | Live |
| IGT-012 | Cards     | `Cards.jsx` per-card `<img src="/ornament-divider.svg" alt="" />` next to the network text — looks informative, marked decorative | 1.1.1 | Images IGT (decorative vs informative) | Moderate | **IGT** (Images) | Live |
| IGT-013 | Bills     | `Bills.jsx` Add-payee dialog opens with `noFocusManagement` (no initial focus, no trap) | 2.4.3 | Modals IGT (focus trap) | Serious | **IGT** (Modals) | Live |
| IGT-014 | Bills     | `Bills.jsx` remove-payee confirm dialog opens with `noEscape` (Esc no longer closes) | 2.1.1 | Modals IGT (Esc-to-close) | Serious | **IGT** (Modals) | Live |
| IGT-015 | Profile   | `Profile.jsx` reset-demo confirm dialog — closing it does NOT return focus to the opener button | 2.4.3 | Modals IGT (focus return) | Serious | **IGT** (Modals) | Live |
| IGT-016 | Home      | `Home.jsx` login card's "Security & Help / Enroll" pair `<div style={{flexDirection:'row-reverse'}}>` — visual order ≠ DOM order | 1.3.2 | Reading Order IGT | Serious | **IGT** (Reading Order) | Live |
| IGT-017 | Dashboard | `Dashboard.jsx` first `.account-card` gets `order: -1` inside `.account-grid` | 1.3.2 | Reading Order IGT | Serious | **IGT** (Reading Order) | Live |
| IGT-018 | Statements | `Statements.jsx` `<div role="button" tabIndex={0}>Open year picker</div>` with onClick only, no onKeyDown | 2.1.1 | Keyboard IGT (custom widget) | Critical | **IGT** (Keyboard) | Live |
| IGT-019 | Transfer  | `Transfer.jsx` Step 1 amount `<input tabIndex={3}>` — positive tabindex forces unnatural order | 2.4.3 | `tabindex` (BP) + Keyboard IGT | Serious | **IGT** (Keyboard) | Live |
| IGT-020 | Cards     | `Cards.jsx` `<div role="button" tabIndex={0}>Order replacement card</div>` with onClick only, no onKeyDown | 2.1.1 | Keyboard IGT (custom widget) | Critical | **IGT** (Keyboard) | Live |
| IGT-021 | Home      | `Home.jsx` `.branch-thumb` `<figure>` where the `<img alt="DQBC Downtown branch">` matches the visible `<figcaption>` text exactly | 1.1.1 | Images IGT + `image-redundant-alt` (BP) | Minor | **IGT** (Images) | Live |
| IGT-022 | Home      | `Home.jsx` `.btn-login` (Log in button) carries positive `tabIndex={5}` — pulls it out of natural DOM tab order | 2.4.3 | Keyboard IGT + `tabindex` (BP) | Serious | **IGT** (Keyboard) | Live |
| IGT-023 | Home      | `Home.jsx` `.sticky-offer-save` `<div role="button" tabIndex={0}>` with `onClick` only — no `onKeyDown` / `onKeyUp` | 2.1.1 | Keyboard IGT (custom widget — hard fail) | Critical | **IGT** (Keyboard) | Live |
...
| ------ | ------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| MT-001 | Open `/#/fintech` (Home) and run a scan                 | Critical: Images must have alternate text (`image-alt`) on the testimonial photo `img` inside `.testimonial-card` (bottom-right of the page) |
...
| MT-011 | Open `/#/fintech` (Home). Scroll past the hero to the "Cash offer up to $500" line inside the bottom cash-offer card — it's visually a heading but semantically a `<div>`. In axe DevTools Pro, run a scan; the finding appears in **Automatic Issues (advanced)**. Note: this rule consumes AI credits, so if your org's credit pool is exhausted, the rule is skipped automatically (the panel will tell you). |
...
| MT-079 | Open `/#/fintech` (Home), scroll to the bottom of the branch footer, run a scan | Critical: Links must have discernible text (`link-name`) on the `.back-to-top` chevron link. |
| MT-080 | Open `/#/fintech` (Home), locate the "Read their stories" link in the bottom-right testimonial card, run a scan | Serious: Interactive controls must not be nested (`nested-interactive`) — a `<button>` lives inside the `<a>`. |
| MT-081 | Open `/#/fintech` (Home), locate the "Terms and conditions apply…" line below the "Open an account" button, run a scan | Serious: Elements must meet minimum colour-contrast ratio thresholds (`color-contrast`) — light-grey text on white lands ~2.4:1. |
| MT-082 | Open `/#/fintech` (Home), locate the sticky offer bar at the bottom of the viewport, run a scan | Critical: Buttons must have discernible text (`button-name`) on the `.sticky-offer-close` icon-only button. |
| MT-083 | Open `/#/fintech` (Home) in a Chrome window with **axe DevTools Pro** installed. Run a scan. The finding appears in **Automatic Issues (advanced)** | Serious: CSS focus visible (`advanced/css-focus-visible`) on the Log in button — the focus outline was removed with no replacement visual state. |
| MT-084 | Open `/#/fintech` (Home). Scroll to the "Featured this month" line above the promo grid — it visually reads as a heading but is a `<div>`. In axe DevTools Pro, run a scan; the finding appears in **Automatic Issues (advanced)**. AI credits required. | Serious: `advanced/heading-markup` on the "Featured this month" styled `<div>`. |
| MT-085 | Open `/#/fintech` (Home). Below the "Open an account" button in the login card is a "View last quarter's offers" link — its parent `<div>` has `aria-hidden="true"`. Run a scan | Serious: ARIA hidden element must not be focusable or contain focusable elements (`aria-hidden-focus`) on `.archive-hint > a`. |
| MT-086 | Open `/#/fintech` (Home). At the end of the "Terms and conditions" fine-print line is a "Read full terms" span with `tabIndex={0}` and an onClick, but no interactive role. Run a scan | Minor (BP): Elements in the focus order need a role appropriate for interactive content (`focus-order-semantics`) on `.fine-print-more`. |
...
These rows target patterns axe DevTools Pro's **Intelligent Guided Tests (IGTs)**
verify. They typically surface in the extension as **Needs Review** /
**Manual Issues** rather than automatic violations — an SE walks the customer
through each guided test in the IGT panel. Where an adjacent axe-core BP rule
also fires (e.g. `landmark-one-main`, `heading-order`, `region`,
`empty-heading`, `image-redundant-alt`, `tabindex`), it is noted.

| ID      | IGT family      | Page URL                          | Where to look                                                                        | How the SE walks a customer through it in the extension                                                                                                                                                                                       |
| ------- | --------------- | --------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IGT-001 | Structure       | `/#/fintech/cards`                | The outer wrapper that contains the page-head and the cards grid (the whole page).   | Open axe DevTools Pro → **IGTs** tab → run the **Structure** guided test. Inspector flags the `role="presentation"` wrapper that strips implicit landmark/region semantics. Walk the customer through "why presentation here breaks structure". |
| IGT-002 | Structure       | `/#/fintech/statements`           | The "Tax help tips" callout above the tax-documents section.                         | Run the **Structure** IGT. It counts `<main>` elements per document; this page has two (AuthLayout's `<main>` + our `<main aria-label="Tax help tips">`). BP rule `landmark-one-main` co-fires on default BP toggle.                          |
| IGT-003 | Structure       | `/#/fintech/help`                 | The grey-soft callout box "Branch hours may vary on bank holidays…" between branch-finder and FAQs. | Run the **Structure** IGT. It surfaces the "orphan region" — a `<section>` that contributes a landmark but has no accessible name (no aria-label / aria-labelledby / heading). BP rule `region` co-fires when BP is on.       |
| IGT-004 | Headings        | `/#/fintech/wealth`               | "A note on advisor availability" callout below the hero.                             | Run the **Headings** IGT. It traverses heading order and flags h1 → h4 jumps (skipped h2, h3). Walk the customer through the heading-tree visualisation. BP rule `heading-order` already covers MT-022 (h1 → h3); IGT-004 widens it to h1 → h4. |
| IGT-005 | Headings        | `/#/fintech/about`                | "DQBC, in a nutshell" headline directly below the MarketingHero (above "Our story"). | Run the **Headings** IGT. It detects the second `<h1>` on the page. BP rule `page-has-heading-one` does NOT fire (the page still has one h1) — `heading-order` is fine too — so this lands as a Manual / IGT-only finding.                   |
| IGT-006 | Headings        | `/#/fintech/profile`              | Just below the saved-notice banner / above the Profile-completeness meter card.      | Run the **Headings** IGT. It flags empty heading elements (`<h2></h2>` with no text). BP rule `empty-heading` co-fires when BP is on.                                                                                                       |
| IGT-007 | Forms           | `/#/fintech/login`                | "Security question" input inside the Sign-in form, between Password row and the Remember-this-device row. | Run the **Forms** IGT. It walks each required field and asks "does a sighted user know this is required?" — no asterisk, no aria-required, no "required" text. SE shows the customer the `required` attribute in DevTools to prove the mismatch.   |
| IGT-008 | Forms           | `/#/fintech/profile`              | The red "Email must include @" line directly under the personal-info name/email row. | Run the **Forms** IGT. The error message is visually adjacent but the Email `<input>` has no `aria-describedby` pointing to it — turn on a screen reader, focus the input, and the customer hears the label only, not the error.            |
| IGT-009 | Forms           | `/#/fintech/transfer` (Step 1)    | "Confirmation preferences" block (Send confirmation email / Send SMS / Skip if duplicate) below the Memo field. | Run the **Forms** IGT. It walks related controls and asks "is this group programmatically labelled?" Show that there's no `<fieldset>` / `<legend>` — an SR user reading each checkbox doesn't hear the group's purpose.                    |
| IGT-010 | Images          | `/#/fintech/dashboard`            | The "Official member portal" strip directly above the balance banner — two small DQBC seals flank the line. | Run the **Images** IGT. It scans the page for `<img>`s and surfaces any with the same src+alt rendered multiple times. Walk the customer through the SR experience hearing "DQBC seal" twice for the same logical badge.                  |
| IGT-011 | Images          | `/#/fintech/about`                | The small branch-photo SVG immediately under "DQBC, in a nutshell" (above "Our story"). | Run the **Images** IGT. It detects images whose visible content is itself text (image-of-text) and whose alt doesn't match the visible text. SE shows the visible text ("DQBC Downtown" signage) vs the alt ("Our story").                     |
| IGT-012 | Images          | `/#/fintech/cards`                | Per-card row beside the Visa network badge — a tiny gold ornament SVG.                | Run the **Images** IGT. It flags images that *look* informative beside text but are marked decorative (`alt=""`). Walk the customer through "is this decorative or informative?" using the visual + adjacent text.                            |
| IGT-013 | Modals          | `/#/fintech/bills`                | Click "Add payee" in the top-right page header. The Add-payee dialog opens.          | Run the **Modals** IGT. Open the dialog, press Tab repeatedly — focus escapes the dialog into the page behind it (no trap). Initial focus also doesn't land on the first input. SE narrates the focus-trap requirement.                     |
| IGT-014 | Modals          | `/#/fintech/bills`                | Open Bills, click any payee's "Remove" link in the saved-payees list. Confirm dialog opens. | Run the **Modals** IGT. Open the dialog and press Esc — nothing happens. SE walks the customer through "Esc-to-close" as one of the required modal behaviours (WAI-ARIA APG dialog pattern).                                              |
| IGT-015 | Modals          | `/#/fintech/profile`              | Scroll to the "Reset demo data" card at the bottom, click the button, then Cancel.   | Run the **Modals** IGT. Open the alertdialog, click Cancel (or Yes, reset) — focus drops to `<body>` instead of returning to the "Reset demo data" button. SE narrates the WCAG 2.4.3 Focus Order requirement.                              |
| IGT-016 | Reading Order   | `/#/fintech` (Home)               | The "Security & Help / Enroll" pair inside the login card.                            | Run the **Reading Order** IGT. Visually "Security & Help" appears first, but DOM has "Enroll" first. SE turns on a screen reader and shows that NVDA/VO read DOM order, mismatching the visual.                                              |
| IGT-017 | Reading Order   | `/#/fintech/dashboard`            | "Your accounts" grid — the first account card visually but second in DOM.            | Run the **Reading Order** IGT. The first child has `order: -1` so it floats to slot 1 visually. SE shows DOM order vs tab order vs visual order divergence.                                                                                |
| IGT-018 | Keyboard        | `/#/fintech/statements`           | "Open year picker" pill below the tax-tips callout / above "Tax documents" section.  | Run the **Keyboard** IGT. Tab to the element — it focuses (has tabindex=0). Press Enter / Space — nothing happens (no onKeyDown). SE compares with a real `<button>` that activates on Enter and Space.                                    |
| IGT-019 | Keyboard        | `/#/fintech/transfer` (Step 1)    | Amount input field.                                                                  | Run the **Keyboard** IGT. Tab through the form — focus jumps to Amount unnaturally because `tabIndex={3}` overrides source order. BP rule `tabindex` co-fires on positive tabindex values when BP is on.                                  |
| IGT-020 | Keyboard        | `/#/fintech/cards`                | "Order replacement card" pill below the cards grid (bottom of the page).             | Run the **Keyboard** IGT. Same pattern as IGT-018: focusable `<div role="button">` with onClick only, no keyboard handler. SE narrates the custom-widget keyboard-pattern requirement (Enter + Space for button role).                     |
| IGT-021 | Images          | `/#/fintech` (Home)               | Small branch thumbnail inside the branch footer, with visible caption reading "DQBC Downtown branch".                                                                                                             | Run the **Images** IGT. The IGT walks the SE through every image on the page; the thumbnail's `alt` text is a verbatim copy of the visible caption, so screen readers announce it twice. Also fires `image-redundant-alt` as an axe-core BP finding.        |
| IGT-022 | Keyboard        | `/#/fintech` (Home)               | Login card's "Log in" primary button.                                                                                                                                                                              | Run the **Keyboard** IGT. Tab-order walkthrough reveals `.btn-login` has `tabIndex={5}` — it jumps out of natural DOM order. SE points out that positive tabindex is a known anti-pattern (screen-reader users lose predictability). Also fires `tabindex` as an axe-core BP finding.  |
| IGT-023 | Keyboard        | `/#/fintech` (Home)               | "★ Save for later" pill in the sticky offer bar at the bottom of the viewport.                                                                                                                                     | Run the **Keyboard** IGT — this one is a **hard fail**, not just needs-review. The IGT simulates Enter and Space keypresses on every focusable element; the fake button div has `onClick` only, so nothing happens on keyboard activation. SE narrates: focusable ≠ operable — always add `onKeyDown`/`onKeyUp` handlers to custom-button divs (or just use `<button>`).                     |
...
**Confirmed 2026-07-27:** the axe MCP server's `analyze` tool (which accepts an `advancedRules` parameter) runs a local, plain axe-core 4.11.0 engine via Playwright — grepping its bundled `axe.js` for any `advanced/`-prefixed rule ID returns zero matches. The 14 catalogued Pro Advanced rows (MT-010/011/012/015/028/029/035/041/048/053/060/066/072/078) are **not present in open-source axe-core at all**; they only exist in Deque's proprietary Axe DevTools Pro backend. So this isn't a credit-pool question when scanning through this MCP tool specifically — the `advancedRules` param has no effect locally regardless of preset (`thorough` vs `precise` produced byte-identical results on Home in this session's sweep). Verifying these 14 rows still requires the licensed browser extension.
...
| PL-005 | Transfer                         | `Transfer.jsx` "← Back to accounts" link in the `.subtitle` row (brand-blue `#1a73e8` on `#f6f8fb` ≈ 4.23:1, just under the 4.5:1 threshold) | 1.4.3 | `color-contrast`      | Serious  | axe-core | Pre-existing |
| PL-006 | Transfer                         | `Transfer.jsx` "Daily transfer limit: $25,000." helper text below the Amount field (`#b8bfca` on white ≈ 1.85:1) | 1.4.3 | `color-contrast`      | Serious  | axe-core | Pre-existing |
...
- **PL-005 / PL-006** surfaced during the 2026-07-27 full-sweep verification (see "Scan-vs-catalog alignment" appendix, July 2026 entry) — neither was previously catalogued. Both read as unintentional leftovers rather than deliberate Phase-2 engineering, same character as PL-001/002. Fix: PL-005 needs a slightly darker blue or a non-color affordance; PL-006 needs a darker grey (e.g. `var(--text-secondary)` instead of the lighter `#b8bfca`).
...
| ID     | Minimal fix                                                                                                              |
| ------ | ------------------------------------------------------------------------------------------------------------------------ |
| MT-001 | Add a descriptive `alt` on the testimonial `<img>` (e.g. `alt="Priya R., DQBC managing director"`), OR — if the photo is purely decorative alongside the visible quote — `alt=""` plus `role="presentation"`.  |
...
| MT-009 | Differentiate the link text: "Learn about Cash Rewards", "Learn about Unlimited Cash", "Learn about Travel Rewards", "Learn about 0% Intro" (or add `aria-label="Learn more about <card name>"` per link). |
...
| MT-011 | Replace the styled `<div>Cash offer up to $500</div>` inside the cash-offer card with an `<h2>` (or `<h3>`) so it participates in the document outline. |
...
| ID     | Steps to reach                                          | Expected finding                                                                          |
| ------ | ------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| MT-025 | Open `/#/fintech` (Home), locate the "Filter by:" `<select>` above the promo-grid on the right side of the hero, run a scan | Critical: Select element must have an accessible name (`select-name`) on the `<select>` — no `<label>`, no `aria-label`, no `aria-labelledby`, no `title`. |
| MT-026 | Open `/#/fintech` (Home), run a scan | Serious: `<svg>` elements with an img role must have an alternative text (`svg-img-alt`) on the small ascending bar-chart svg next to the first promo card's "$200" badge. |
| MT-027 | Open `/#/fintech` (Home), scroll to the "Popular products" box below the login card, run a scan | Critical: Certain ARIA roles must contain particular children (`aria-required-children`) on the `<div role="listbox">` — the children are bare `<div>`s with no `role="option"`. |
...
| MT-030 | Open `/#/fintech` (Home), run a scan | Critical: ARIA attribute value must be valid (`aria-valid-attr-value`) on the "Show additional login options" disclosure `<button aria-expanded="yes">` at the bottom of the login card. `aria-expanded` accepts only `true` / `false`. |
...
| ID     | Minimal fix                                                                                                              |
| ------ | ------------------------------------------------------------------------------------------------------------------------ |
| MT-025 | Add a `<label htmlFor="card-filter">Filter by</label>` and give the `<select>` matching `id="card-filter"`. (Or, if a visible label is undesirable, add `aria-label="Filter cards by type"` on the `<select>` itself.) |
...
4. **MT-016** `scope-attr-valid` (Statements) — **root cause confirmed 2026-07-27** by reading the rule config directly out of the installed axe-core 4.11.0 bundle: this rule's `tags` are `['cat.tables', 'best-practice']` — it carries **no WCAG tag at all** in this version (it was WCAG 1.3.1-tagged in the 4.10.2 sweep this catalog was originally built against). It is now Best-Practice-only, so it will never appear under the default WCAG-only toggle regardless of markup — this is a rule reclassification between axe-core versions, not a markup defect. No code change needed or possible; verify with **Best Practices: ON**. Catalog's WCAG/Tool columns for MT-016 updated accordingly.
5. **Keyboard IGT automation gap (IGT-018/019/020/022/023)** — confirmed 2026-07-27 via the axe MCP server's `igt` tool on Home and Cards: the automated Keyboard IGT correctly records each element's tab-stop data (accurate `tabIndex`, accurate `role`) but its `issues` output never flags either (a) a positive-`tabIndex` value out of natural order (IGT-019 Transfer, IGT-022 Home), or (b) a focusable `role="button"` div with `onClick` but no `onKeyDown`/`onKeyUp` (IGT-018 Statements, IGT-020 Cards, IGT-023 Home) — even though the element is present in the raw tab-stop list with the anomalous property. The tool's automated pass only surfaces role-mismatch and missing-focus-indicator patterns (which is how it correctly caught MT-086 on Home and MT-003/MT-046 on Profile). These 5 rows still require the full axe DevTools Pro extension's Keyboard IGT script — they are not verifiable through this MCP tool as currently built. Separately, the automated Keyboard IGT on `/fintech/transfer` timed out twice (200s) in this session, likely due to the page's density of custom ARIA widgets requiring per-element AI classification; IGT-019 there is unconfirmed either way and needs manual verification.
...
- **Pro Advanced (Automatic Issues — advanced bucket):** MT-010, MT-011, MT-012, MT-015 (gradient fallback), MT-028, MT-029, MT-035, MT-041, MT-048, MT-053, MT-060, MT-066, MT-072, MT-078, MT-087, MT-089, MT-091, MT-093.
...
Full sweep across all **19** DQBC pages (the 15 covered by the June appendix above, plus `/checking`, `/savings`, `/credit-cards`, `/mortgages` which didn't exist yet at the time of that sweep), using the axe MCP server's `analyze` (WCAG 2.2 AA, `advancedRules: thorough`) and `igt` (`keyboard`) tools per CLAUDE.md's mandated workflow. Confirms the June appendix's per-page counts still hold on axe-core 4.11.0, with the following deltas:

- **MT-016 reclassified, not broken** — see Known gaps item 4 above. Root-caused via the axe-core source directly, not by guessing.
- **MT-080 fixed this session** — `nested-interactive`'s matcher (`nestedInteractiveMatches`) only evaluates elements whose ARIA role has `childrenPresentational: true` (`button`, `checkbox`, `tab`, etc.) — a bare `<a>` (role `link`) doesn't qualify, so a `<button>` nested in a real link structurally cannot trip this rule in axe-core 4.11, regardless of the inner button's markup. Added `role="button"` to the outer `<Link>` in `Home.jsx` (a plausible real-world "link redecorated as a button" mistake) — confirmed firing via `analyze` scoped to `.testimonial-link`.
- **PL-005 / PL-006** — 2 previously-uncatalogued `color-contrast` violations found on Transfer (see Pre-existing Phase 1 issues table above).
- **Keyboard IGT tooling gap** — see Known gaps item 5 above. Confirmed on Home and Cards by inspecting the raw `igtElements` tab-stop data.
- **Pro Advanced rules** — confirmed via direct inspection of the installed axe-core 4.11.0 bundle that none of the `advanced/*` rule IDs exist in open-source axe-core at all (see "Pro Advanced verification caveat" above). The `advancedRules` parameter on this MCP's `analyze` tool has no effect locally.
- **MT-087 through MT-094 added** — the 4 previously-empty product pages (`/checking`, `/savings`, `/credit-cards`, `/mortgages`) each got one Pro-Advanced-tagged issue (heading-markup, text-contrast, css-focus-visible, image-informative-has-alt respectively — rotating through the same rule types already used elsewhere) plus one `focus-order-semantics` custom-widget pattern. All 5 are provably automatable via the same mechanism: a keyboard-focusable (`tabIndex={0}`) element with an `onClick` but no interactive ARIA role, which the automated Keyboard IGT flags as `aria-role-missing` regardless of what the element visually looks like. Rather than repeat MT-086's exact link look-alike 4 more times, each instance got a genuinely different visual/semantic shape so the IGT's AI role-mismatch reasoning diversifies too — confirmed live on all 4:
  - MT-088 (Checking) — underlined text → AI suggests `role="link"`
  - MT-090 (Savings) — solid pill button → AI suggests `role="button"`
  - MT-092 (Credit Cards) — checkbox-square + checkmark → AI suggests `role="checkbox"`
  - MT-094 (Mortgages) — segmented-control tab pair (2 tab stops) → AI suggests `role="tab"` on the first node

  This is the same detection path that already worked for MT-086 on Home, so these instances give the Keyboard IGT demo broader page coverage AND a wider range of role-mismatch reasoning to show a customer, without relying on the onKeyDown/tabindex detection gap noted above. The 4 Pro-Advanced additions still need the licensed extension to verify, same as the rest of the Pro Advanced set.

- **MT-095 through MT-098 added** — a further pass at the same 4 product pages, this time targeting the **default-toggle axe-core scan** (no BP/Experimental needed) with rule types not yet used elsewhere in the catalog, found by reading axe-core's rule config directly rather than assuming:
  - MT-096 (Checking) / MT-098 (Mortgages) — `aria-command-name` (icon-only `role="button"` with no accessible name). Confirmed firing via `analyze` on both pages under default toggles. Distinct from `button-name`/`link-name`, which only cover native `<button>`/`<a>` elements — this is the ARIA-role equivalent, previously absent from the catalog.
  - MT-097 (Savings) — `svg-img-alt`, a second instance of an existing rule type (first used on Home's MT-026) but a new page/context (an FDIC-insured badge icon). Confirmed firing via `analyze`.
  - MT-095 (Credit Cards) — `duplicate-id-aria`, attempted as a fourth fresh rule type (a reused anchor `id` prop causing a genuine duplicate `id="compare-heading"`, confirmed via direct Playwright DOM inspection). **Correction during verification**: `analyze` returned empty on this page. Reading the rule's definition in the installed axe-core bundle explains why — this rule ships with `reviewOnFail: true`, meaning it is *always* reported as a Needs Review finding, never a confirmed violation, regardless of markup. Same bucket as MT-009. The defect is real (verified independently via DOM inspection) but will not appear in this MCP's `analyze` output; corrected the catalog row and inline comment to say so rather than leave the original overclaim ("fires under the DEFAULT scan") standing. Also checked `aria-command-name` and `svg-img-alt`'s configs to confirm neither carries `reviewOnFail` — their live-fire results above are trustworthy for that reason, not just because a scan happened to return non-empty.
  - Considered and rejected for this batch: `duplicate-id` / `duplicate-id-active` — both ship `enabled: false` in this axe-core version (structurally cannot fire regardless of toggles); `css-orientation-lock` and `table-fake-caption` — both carry an `experimental` tag with no corresponding toggle exposed by this MCP's `analyze` tool.
